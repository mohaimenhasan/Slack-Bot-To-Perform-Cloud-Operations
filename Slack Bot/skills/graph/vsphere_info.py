...
def printToScreen(name, capacity,uncommittedSpace,freeSpace,freeSpacePercentage):
    strz = "\n"
    strz += "##################################################\n"
    strz += "Datastore name: " +  name + "\n"
    strz += "Capacity: " + str(humanize.naturalsize(capacity, binary=True)) + "\n"
    if uncommittedSpace is not None:
        provisionedSpace = (capacity - freeSpace) + uncommittedSpace
        strz += "Provisioned space: " + str(humanize.naturalsize(provisionedSpace,
                                                          binary=True)) + "\n"
    strz += "Free space: " + str(humanize.naturalsize(freeSpace, binary=True)) + "\n"
    strz += "Free space percentage: " + str(freeSpacePercentage) + "%\n" 
    strz += "##################################################\n"
    return strz    

def getJsonObject(name, capacity,uncommittedSpace,freeSpace,freeSpacePercentage):
    
    data = {}
    data['Datastore name'] = name
    data['Capacity'] = str(humanize.naturalsize(capacity, binary=True))
    
    provisionedSpace = (capacity - freeSpace) + uncommittedSpace
    data['Provisioned space'] = str(humanize.naturalsize(provisionedSpace, binary=True))
    data['Free space'] = str(humanize.naturalsize(freeSpace, binary=True))
    data['Free space percentage'] = str(freeSpacePercentage)
...
def printDatastoreInformation(datastore):
    try:
        ...
        return getJsonObject(summary.name,capacity,freeSpace,uncommittedSpace,freeSpacePercentage)
    ...
    for ds in datastores:
        try:
            if ds.summary.name == datastoreName:
                ...
                freeSpacePercentage = (float(freeSpace) / capacity) * 100
                f1 = open('./output', 'w')
                f1.write(getJsonObject(summary.name,capacity,freeSpace,uncommittedSpace,freeSpacePercentage))
                f1.close()
        except Exception as error:
...
def printDataStore(datacenter):
    ...
    for ds in datastores:
        print(i)
        jsonData = printDatastoreInformation(ds)
        if i == len(datastores) - 1:
            f1.write(jsonData)
        else:
            f1.write(jsonData + ",")
        i+=1
